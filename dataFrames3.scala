...
val dfPeople = spark// Our SparkSession & Entry Point
  .read// Our DataFrameReader
  .option("sep",":")
  .option("header", "true")//Se toma la opcion del header        
  .option("inferSchema", "true")//Se infiere el esquema de la tabla
  .csv(sourceFile)//Se lee el archivo csv
  
dfPeople.printSchema()

import org.apache.spark.sql.functions._

val dfPeopleFinal = dfPeople
  .withColumn("firstName", upper($"firstName"))
  .withColumn("middleName", upper($"middleName"))
  .withColumn("lastName", upper($"lastName"))
  .withColumn("ssn", regexp_replace($"ssn","-",""))
  .withColumn("birthDate", $"birthDate".cast("date"))
  .distinct()

println("Cant registros unicos "+dfPeopleFinal.count())

println("Cant registros totales "+dfPeople.count())
